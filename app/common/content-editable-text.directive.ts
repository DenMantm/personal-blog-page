...
@Directive({
  selector: '[contenteditableModelText]',
  host: {
    //'(keyup)': 'onKeyup()',
    '(input)': 'onKeyup()'
  }
})
export class ContenteditableModelText {
  @Input('contenteditableModelText') model: string;
  @Output('contenteditableModelTextChange') update = new EventEmitter();
